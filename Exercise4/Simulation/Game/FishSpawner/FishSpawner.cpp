...
namespace Simulation {
    ...
    void FishSpawner::SpawnFish() {
        ...
        auto asteroidRenderComponent = std::make_shared<Simulation::FishRenderComponent>(obj);
        auto asteroidUpdateComponent = std::make_shared<Simulation::FishUpdateComponent>(obj);
    ...
    std::vector<std::shared_ptr<MyEngine::GameObject>> FishSpawner::ChecksFishCollisionsWithLasers() {
        ...
        for(int i = 0; i < engine->gameObjects.size(); i++) {
            ...
            if(asteroidGameObject->GetName() == "fish") {
                for(int j = 0; j < engine->gameObjects.size(); j++) {
                    if(engine->gameObjects[j]->GetName() == "bullet") {
                        ...
                        bool areTwoObjectsColliding = distanceBetweenLaserAndAsteroid <= pow(fishRadius + bulletGameObject->radius, 2);
